...
const FlopPurchasesByCategoryList: React.FC = () => {
  const { categories, loading } = usePurchasesByCategoryContext();

  // Filtrage pour exclure les catégories inconnues ou non pertinentes
  const filteredPurchases = categories.filter(
    (category) =>
      category.category.toLowerCase() !== "inconnu" &&
      category.category.toLowerCase() !== "divers catégories"
  );

  // Tri des catégories par montant d'achat croissant (les plus petits en premier)
  const sortedCategories = [...filteredPurchases].sort((a, b) => a.cost - b.cost);
  ...
  return (
    <div className="bg-gradient-to-br from-white to-gray-50 shadow-xl rounded-lg p-4 h-full">
      <div className="flex items-center gap-4 mb-4">
        ...
        <h2 className="text-lg font-bold text-gray-800">Flop 10 Achats par Catégorie</h2>
      </div>
      {loading ? (
              ...
              >
                ...
                <div className="flex flex-col items-end text-xs">
                  ...
                  <span className="text-gray-500">
                    Achat: {formatLargeNumber(category.cost)}
                  </span>
                </div>
              </li>
            ))}
...
export default FlopPurchasesByCategoryList;