...
import { usePurchasesByUniverseContext } from "@/contexts/sell-in/PurchasesByUniverseContext";
...
const PurchasesByUniverseChart: React.FC = () => {
  const { handleClearAllFilters, setFilters } = useFilterContext();
  const { universes, loading } = usePurchasesByUniverseContext();
  ...
  const chartData = {
    ...
    datasets: [
      {
        label: "Coût des achats par Univers (en €)",
        data: filteredUniverses.map((u) => u.cost),
  ...
  const chartOptions = {
    ...
    onClick: (event: any, elements: any) => {
      if (elements.length > 0) {
        ...
        const selectedUniverse = chartData.labels[index];
  ...
  return (
    <div className="bg-gradient-to-br from-white to-gray-50 shadow-xl rounded-lg p-8 h-full">
      <div className="flex items-center gap-4 mb-4">
        ...
        <h2 className="text-xl font-bold text-gray-800">Graphique des Achats par Univers</h2>
      </div>
      {loading ? (
...
export default PurchasesByUniverseChart;