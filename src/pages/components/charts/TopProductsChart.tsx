...
const TopProductsChart: React.FC<TopProductsChartProps> = ({ groupedSales }) => {
  const [chartMetric, setChartMetric] = useState<ChartMetric>('quantity'); // Métrique par défaut

  // Calcul des valeurs pour chaque produit en fonction de la métrique choisie
  const productMap: Record<string, number> = useMemo(() => {
    ...
    groupedSales.forEach(sale => {
      ...
      if (!isFinite(value)) {
        console.warn(`Valeur non finie pour le produit "${productName}" avec la métrique "${chartMetric}":`, value);
        value = 0; // Optionnel : Réinitialiser à 0 ou une autre valeur par défaut
      }

      map[productName] = (map[productName] || 0) + value;
    });

    console.log('Product Map:', map); // Ajouter un log pour vérifier les valeurs
  ...
  const topProducts = useMemo(() => {
    const productsArray = Object.entries(productMap).map(([productName, val]) => ({ productName, val }));
    productsArray.sort((a, b) => b.val - a.val);
    return productsArray.slice(0, 10); // Top 10 produits
  }, [productMap]);

  // Calculer les trois pires produits globalement
  const worstProducts = useMemo(() => {
    const productsArray = Object.entries(productMap).map(([productName, val]) => ({ productName, val }));
    productsArray.sort((a, b) => a.val - b.val); // Trier par ordre croissant
    return productsArray.slice(0, 3); // Trois pires produits
  }, [productMap]);

  const chartLabels = topProducts.map(c => truncate(c.productName, 20)); // Limiter à 20 caractères
  const chartData = topProducts.map(c => c.val);
    ...
    datasets: [
      {
        ...
        backgroundColor: chartData.map((_, i) => 
          i === 0 ? 'rgba(54, 162, 235, 0.6)' : 'rgba(75, 192, 192, 0.6)' // Exemple : couleur différente pour le premier élément
        ),
        borderColor: chartData.map((_, i) => 
          i === 0 ? 'rgba(54, 162, 235, 1)' : 'rgba(75, 192, 192, 1)' // Couleur correspondante
        ),
        borderWidth: 1,
      }
    ]
      ...
      <div className="bg-white shadow-lg rounded-lg p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center">
            ...
            <h2 className="text-xl font-semibold text-primary">Top 10 Produits</h2>
          </div>
        ...
        {/* Cartes des Trois Pires Produits */}
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            {worstProducts.map((product, index) => (
              <div key={index} className="flex-1 bg-red-100 shadow rounded-lg p-4 flex items-center">
                ...
                <div>
                  <p className="text-sm font-medium text-red-500">Pire Produit</p>
                  <p className="mt-1 text-lg font-semibold text-red-700 break-words" title={product.productName}>
                    {product.productName} {/* Afficher le nom complet sans troncature */}
                  </p>
                  <p className="mt-1 text-md text-red-600">
                    {chartMetric === 'cattc' || chartMetric === 'margin' 
                      ? new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(product.val)
                      : product.val.toLocaleString('fr-FR')}
                  </p>
                </div>
              </div>
            ))}
