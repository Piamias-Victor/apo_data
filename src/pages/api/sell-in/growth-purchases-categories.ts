...
interface GrowthPurchasesCategoriesResponse {
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<GrowthPurchasesCategoriesResponse | { error: string }>
) {
  try {
    ...
    const growthPurchasesCategories = result.rows.map((row) => ({
    ...
    res.status(200).json({ growthPurchasesCategories });
  } catch (error) {
    console.error("Erreur lors du calcul du prix d'achat moyen par catégorie :", error);
