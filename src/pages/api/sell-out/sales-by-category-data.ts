...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<SalesByCategoryResponse | { error: string }>
) {
  try {
    const {
      pharmacy,
    ...
    // Requête SQL pour récupérer les ventes par catégorie avec la marge
      const query = `
    SELECT 
      gp.category,
    ...
    HAVING SUM(s.quantity * COALESCE(i.price_with_tax, 0)) > 0 -- Exclure les catégories avec un CA nul ou négatif
