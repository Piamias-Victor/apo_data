// src/pages/api/sell-out/regression-categories-data.ts

import type { NextApiRequest, NextApiResponse } from "next";
import pool from "@/libs/fetch/db";

/** Interface pour la réponse JSON */
interface RegressionCategoriesResponse {
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<RegressionCategoriesResponse | { error: string }>
) {
  try {
    const {
      pharmacy,
    ...
    const startLast3Months = new Date(now.getFullYear(), now.getMonth() - 3, 1)
      .toISOString()
      .split("T")[0];
    const endLast3Months = new Date(now.getFullYear(), now.getMonth(), 0)
      .toISOString()
      .split("T")[0];
    const startPrevious3Months = new Date(
      now.getFullYear(),
      now.getMonth() - 6,
      1
    )
      .toISOString()
      .split("T")[0];
    const endPrevious3Months = new Date(
      now.getFullYear(),
      now.getMonth() - 3,
      0
    )
    ...
    // On ajoute les 4 dates (période précédente, période courante) à 'values'
    values.push(
      startPrevious3Months,
      endPrevious3Months,
      startLast3Months,
      endLast3Months
    );

    // 3) Requête SQL : agrégation par gp.category
    const query = `
      WITH categories_regression AS (
        SELECT
      ...
      SELECT
        ...
        total_revenue
      FROM categories_regression
      WHERE current_quantity > 0
      -- Filtrer si besoin : ex. AND regression_rate < 0 pour “vraie” régression
    ...
    // 5) Formatage final
    const regressionCategories = result.rows.map((row) => ({
    ...
    // 6) Réponse JSON
    res.status(200).json({ regressionCategories });
  } catch (error) {
    console.error("Erreur lors de l'analyse de la régression des catégories :", error);
