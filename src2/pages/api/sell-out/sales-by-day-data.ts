...
interface SalesByDayResponse {
  days: string[]; // Liste des jours au format "YYYY-MM-DD"
  quantities: number[]; // Quantité vendue par jour
  revenues: number[]; // Revenu total par jour
  margins: number[]; // Marge totale par jour
}

export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<SalesByDayResponse | { error: string }>
) {
  try {
    const {
      pharmacy,
      ...
      product,
      selectedCategory,
      limitDays, // Optionnel : Limite le nombre de jours retournés
    } = req.query;

    const whereClauses: string[] = [
      "s.quantity > 0",
      "s.date >= DATE_TRUNC('month', CURRENT_DATE)", // Récupère uniquement le mois en cours
    ];
    const values: any[] = [];
    let paramIndex = 1;
    ...
    const query = `
      ...
      ORDER BY TO_CHAR(s.date, 'YYYY-MM-DD')
      ${limitDays ? `LIMIT $${paramIndex}` : ""};
    `;

    if (limitDays) {
      values.push(parseInt(limitDays as string, 10));
    }
    ...
    res.status(200).json({ days, quantities, revenues, margins });
  } catch (error) {
    console.error("Erreur lors de la récupération des ventes par jour avec marges :", error);
